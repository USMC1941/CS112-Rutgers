...
public class Heap<T extends Comparable<T>> {
	...
	private void siftUp() {
		...
		while (k > 0) {
			...
			if (mode == MAX_HEAP && kid.compareTo(parent) > 0 || mode == MIN_HEAP && kid.compareTo(parent) < 0) {
	...
	private void siftDown() {
		...
		while (l < items.size()) {
			...
			int max = l;
			if (r < items.size() && ((mode == MAX_HEAP && items.get(r).compareTo(items.get(l)) > 0) || (mode == MIN_HEAP && items.get(r).compareTo(items.get(l)) < 0))) {
				max = r;
			}
			if ((mode == MAX_HEAP && items.get(max).compareTo(items.get(k)) > 0) || (mode == MIN_HEAP && items.get(max).compareTo(items.get(k)) < 0)) {
				// swap
