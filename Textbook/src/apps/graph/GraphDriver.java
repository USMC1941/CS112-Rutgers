...
/**
 * A simple driver application to exercise the undirected graph class, UndirGraph.
 * 
...
public class GraphDriver {  
	 ...
	 */
	static UndirGraph<String> G = new UndirGraph<String>();
	...
	throws IOException {
		...
		G.addEdge(G.vertexNumberOf(flabel),
				new Neighbor(G.vertexNumberOf(tlabel)));
	}       
	...
	throws IOException {
		...
		Neighbor nbr = G.firstNeighbor(v);
		while (nbr != null) {
			System.out.println(G.vertexInfoOf(nbr.vertexNumber));
			nbr = G.nextNeighbor(v);
		}    
	}
	...
	throws IOException {
		...
		if (G.containsEdge(srcVert, new Neighbor(destVert))) {
