...
public class LittleSearchEngine {
    ...
    public ArrayList<Integer> insertLastOccurrence(ArrayList<Occurrence> occs) {
        ...
        while (true) {                                              // Perform binary search for place to insert
            ...
            //
            if (occMidpoint.frequency == frequencyToInsert) {       // Hit the midpoint
                indexToInsert = midpoint;
                break;
            }
            else if (occMidpoint.frequency < frequencyToInsert) {   // Searching in left
                upperBound = midpoint - 1;                          // It's in the left of the midpoint
                if (lowerBound > upperBound) {
                    indexToInsert = midpoint;                       // At this time, could not find while searching to left in lowerBound = upperBound. Insert into current
                    break;
                }
            }
                ...
                if (lowerBound > upperBound) {
                    indexToInsert = midpoint + 1;                   // At this time, could not find while searching to the right at lowerBound = upperBound. Insert into the next one to the right.
                    break;
                }
            }
        }
    ...
    public ArrayList<String> top5search(String kw1, String kw2) {
        ...
        int index1 = (occ1 == null) ? -1 : 0;                          // -1 means stop searching, because either keyword not found in document or reach the end of the list. Otherwise, start from 0.
        int index2 = (occ2 == null) ? -1 : 0;
        //
        while ( (lstOutput.size() < 5) && (index1 >= 0 || index2 >= 0) ) {  // Continue if the limit (5) is not reached yet and there is still something there
            Occurrence first  = (index1 >= 0) ? occ1.get(index1) : null;
            Occurrence second = (index2 >= 0) ? occ2 != null ? occ2.get(index2) : null : null;
            //
            Occurrence oneOcc;                                          // Get one with highest frequency from the head of occ1 and occ2. occ1 have higher priority if lists have same frequency
            if (first == null) {                                        // Only first is null
                ...
                if (first.frequency >= second.frequency) {              // First takes priority (thus the = here) if first and second have equal frequencies
                    oneOcc = first;
                    index1++;
                }
                else {
            ...
            if (oneOcc != null) {
                if (!docReturned.containsKey(oneOcc.document)) {
                    ...
                    // Do nothing. Doc is already in the output list
                }
            }
            //
